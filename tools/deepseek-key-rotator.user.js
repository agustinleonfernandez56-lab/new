...
(function () {
  ...
  const CONFIG = {
    ...
    secret: "cf94d69047a67ad5d62ef27eebc5fa4993205848a9fe40a0",
    provider: "deepseek",
  ...
  function grabBearer(value) {
    if (typeof value === "string" && /^Bearer\s+\S+/.test(value))
      bearer = value;
  }
    ...
    GM_xmlhttpRequest({
      ...
      onload(res) {
        ...
        if (res.status >= 200 && res.status < 300) {
          ...
          ui(
            `❌ Сервер отклонил: ${res.status}\n${String(res.responseText).slice(0, 120)}`,
            "#a50e0e",
          );
          console.error(
            "[key-rotator] отказ сервера:",
            res.status,
            res.responseText,
          );
        }
  ...
  window.fetch = function (...args) {
    ...
    try {
      ...
      if (h)
        grabBearer(
          h.get ? h.get("Authorization") : h.Authorization || h.authorization,
        );
    } catch {
      /* ignore */
    }
    if (CONFIG.recon)
      console.log("[recon fetch]", (args[1] && args[1].method) || "GET", url);
  ...
  async function dsCall(payload) {
    if (!bearer)
      throw new Error("нет токена сессии (открой/обнови страницу api_keys)");
    ...
    if (!res.ok)
      throw new Error(
        `DeepSeek ${res.status}: ${JSON.stringify(json).slice(0, 160)}`,
      );
    return json;
  }
      ...
      GM_xmlhttpRequest({
        ...
        onload: (r) =>
          r.status >= 200 && r.status < 300
            ...
            : reject(
                new Error(
                  `сервер ${r.status}: ${String(r.responseText).slice(0, 120)}`,
                ),
              ),
  ...
  async function rotateNow(reason) {
    ...
    if (CONFIG.secret === SECRET_PLACEHOLDER) {
      ui("⚠️ Впиши секрет в CONFIG.secret", "#a50e0e");
      return;
    }
    if (!bearer) {
      ui("⏳ Жду токен сессии — открой вкладку API keys", "#8a6d00");
      return;
    }
    rotating = true;
    try {
      ...
      if (!ak || !ak.sensitive_id)
        throw new Error(
          `create: неожиданный ответ ${JSON.stringify(cr).slice(0, 160)}`,
        );
      ...
      if (prev && prev.tracking_id && prev.tracking_id !== ak.tracking_id) {
        ...
        try {
          ...
          console.warn(
            "[key-rotator] удаление прошлого ключа не удалось:",
            e.message,
          );
        }
      }
  ...
  window.addEventListener("load", () => {
    ...
    ui(
      noSecret
        ...
        : "Слежу за ключом. Клик — ротировать.",
      noSecret ? "#a50e0e" : "#333",
    );
  ...
  console.log(
    "[key-rotator] активен, autoRotate =",
    CONFIG.autoRotate,
    "recon =",
    CONFIG.recon,
  );